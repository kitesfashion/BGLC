        ...
        if (!empty(@$userMenuAction)) {
            foreach ($userMenuAction as $action) {
                if (in_array($action->id, @$rolePermission)) {
                    ...
                    $data_link .= '<a id="cancel_'.$id.'"  onclick="DeleteData('."'".$id."'".')" data-toggle="tooltip" data-original-title="'.$action->actionName.'" data-id="'.$id.'"  data-token="{{ csrf_token() }}"> <i class="fa fa-trash text-danger"></i> </a>';
                    }
            ...
            foreach ($userMenuAction as $action) {
                if (in_array($action->id, @$rolePermission)) {
                    if($action->menuType == 3){
                        ...
                        $data_link .= '<span class="switchery-demo m-b-30" id="status_'.$id.'" onclick="statusChange('.$id.')">
